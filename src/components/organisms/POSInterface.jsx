...
return (
            ...
            <div className="flex items-center space-x-4">
                ...
                <div className="flex bg-gray-100 rounded-lg p-1">
                    <Button
                        ...
                        <ApperIcon name="ShoppingCart" className="h-4 w-4 mr-1" />Sale
                                      </Button>
                    <Button
                        ...
                        className="rounded-md">
                        <ApperIcon name="RotateCcw" className="h-4 w-4 mr-1" />Return
                                      </Button>
                    <Button
                        ...
                        className="rounded-md">
                        <ApperIcon name="RefreshCw" className="h-4 w-4 mr-1" />Exchange
                                      </Button>
                </div>
            </div>
            <Badge
                variant={transactionMode === "sale" ? "success" : transactionMode === "return" ? "warning" : "info"}
                size="lg">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {(transactionMode === "return" || transactionMode === "exchange") && <div className="lg:col-span-3">
            <Card className="p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">
                    {transactionMode === "return" ? "Process Return" : "Process Exchange"}
                </h2>
                {!originalTransaction ? <div className="space-y-4">
                    <div className="flex space-x-4">
                        ...
                        <Button
                            ...
                            className="mt-6">
                            <ApperIcon name="Search" className="h-4 w-4 mr-1" />Lookup
                                                </Button>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div>
                            ...
                            <p className="text-sm text-gray-600">Original Total: ${originalTransaction.total.toFixed(2)}
                            </p>
                        </div>
                        <Button
                            variant="ghost"
                    ...
                    <div>
                        ...
                        {getSelectedReturnItems().length > 0 && <div className="mt-4 p-3 bg-gray-50 rounded-lg">
        ...
        {(transactionMode === "sale" || transactionMode === "exchange" && originalTransaction) && <div className="lg:col-span-2">
            <Card className="p-6">
                ...
                {filteredProducts.length === 0 ? <Empty
                    ...
                    icon="Package" /> : <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                    {filteredProducts.map((product, index) => <motion.div
                        ...
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <Badge
                                variant={product.totalStock <= product.reorderPoint ? "warning" : "success"}
                                size="sm">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="text-lg font-semibold text-gray-900">${product.basePrice.toFixed(2)}
                            </div>
                            <div className="flex flex-wrap gap-1">
                                {product.availableSizes.slice(0, 3).map(size => <Badge key={size} variant="default" size="sm">
                                    {size}
                                </Badge>)}
                            </div>
                        </div>
        ...
        <div className="lg:col-span-1">
            <Card className="p-6 sticky top-6">
                <div className="flex items-center justify-between mb-6">
                    ...
                    {cart.length > 0 && <Button variant="ghost" size="sm" onClick={clearCart}>Clear
                                      </Button>}
                </div>
...
{/* Promotions */}
                    ...
                    <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-3">
                            <Button
                                ...
                                <ApperIcon name="Gift" className="h-4 w-4 mr-1" />Sell Gift Card
                                                  </Button>
                            <Button
                                variant="secondary"
                                size="sm"
                                ...
                                className="justify-center">
                                <ApperIcon name="CreditCard" className="h-4 w-4 mr-1" />Check Balance
                                                  </Button>
                        </div>
                        {/* Gift Card Purchase */}
                        {showGiftCardPurchase && <div className="space-y-3 p-3 bg-white rounded border">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <Button
                                    ...
                                    size="sm"
                                    onClick={() => setPaymentMethod("card")}>Card
                                                          </Button>
                            </div>
                            ...
                            <Button
                                ...
                                size="sm"
                                className="w-full"
                                disabled={!giftCardAmount || !recipientName || paymentMethod === "cash" && parseFloat(customerPaid) < parseFloat(giftCardAmount)}>Create Gift Card
                                                    </Button>
                        </div>}
                        {/* Gift Card Balance Check */}
                        {showGiftCardRedeem && <div className="space-y-3 p-3 bg-white rounded border">
                            ...
                            <div className="flex space-x-2">
                                ...
                                <Button
                                    ...
                                    disabled={!giftCardCode.trim()}>Check
                                                          </Button>
                            </div>
                            {giftCardBalance !== null && <div className="p-2 bg-success/10 rounded text-sm text-success">Available Balance: ${giftCardBalance.toFixed(2)}
                            </div>}
                        </div>}
                    </div>
                    ...
                    <div className="mb-4">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            <Button
                                ...
                                className="justify-center">Cash
                                                  </Button>
                            <Button
                                variant={paymentMethod === "card" ? "primary" : "secondary"}
                                size="sm"
                                onClick={() => setPaymentMethod("card")}
                                className="justify-center">Card
                                                  </Button>
                            <Button
                                variant={paymentMethod === "gift_card" ? "primary" : "secondary"}
                                size="sm"
                                onClick={() => setPaymentMethod("gift_card")}
                                className="justify-center">Gift Card
                                                  </Button>
                        </div>
                    </div>
                    ...
                    {paymentMethod === "gift_card" && <div className="mb-4">
                        ...
                        {giftCardBalance !== null && <div
                            className={`mt-2 p-2 rounded text-sm ${giftCardBalance >= calculateTotal() ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>Available: ${giftCardBalance.toFixed(2)}| 
                    ...
                    {transactionMode === "return" ? <Button
                        ...
                        <ApperIcon name="RotateCcw" className="h-4 w-4 mr-2" />Process Return (${calculateReturnTotal().toFixed(2)})
                                        </Button> : transactionMode === "exchange" ? <>
                        {originalTransaction && <div className="mb-4 p-3 bg-gray-50 rounded-lg space-y-2">
                            <div className="flex justify-between text-sm">
                            ...
                            <div className="flex justify-between font-semibold pt-2 border-t">
                                ...
                                <span className={calculateExchangeTotal() >= 0 ? "text-error" : "text-success"}>${Math.abs(calculateExchangeTotal()).toFixed(2)}
                                </span>
                            </div>
                        </div>}
                        <Button
                            onClick={processTransaction}
                            className="w-full"
                            disabled={!originalTransaction || getSelectedReturnItems().length === 0 || cart.length === 0 || calculateExchangeTotal() > 0 && paymentMethod === "cash" && parseFloat(customerPaid) < calculateExchangeTotal()}>
                            <ApperIcon name="RefreshCw" className="h-4 w-4 mr-2" />Process Exchange
                                              </Button>
                    </> : <Button
                        onClick={processTransaction}
                        className="w-full"
                        disabled={cart.length === 0 || paymentMethod === "cash" && parseFloat(customerPaid) < calculateTotal() || paymentMethod === "gift_card" && (!giftCardCode.trim() || giftCardBalance !== null && giftCardBalance < calculateTotal())}>Process Payment
                                        </Button>}
                </>}
            </Card>
        </div>
